...
async def insert_product_price(message: types.Message, state: FSMContext):
    ...
    await ProductForm.next()
    await bot.send_message(chat_id=message.from_user.id,
                           text=f"What product photo id to add for?\n")
...
async def insert_product_photo(message: types.Message, state: FSMContext):
    ...
    try:
        session.add(
            Product(name=data.get('name'), age_category=data.get('age_category'), gender=data.get('gender'), size=data.get('size'), price=data.get('price'),
